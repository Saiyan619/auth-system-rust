...
pub async fn send_mail(
    ...
    let cred = Credentials::new(smtp_username.clone(), smtp_password.clone());

    let mailer = SmtpTransport::starttls_relay(&smtp_server)?.credentials(cred).port(smtp_port).build();
